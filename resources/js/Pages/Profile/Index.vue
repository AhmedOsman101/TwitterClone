  ...
  <AuthenticatedLayout>
    ...
    <section class="grid">
      ...
      <Tweet
          ...
          v-if="profileStore.posts.length !== 0 && globalState.activeProfileOption.value === 'posts'"
          :key="tweet.id"
          :tweet="tweet"
      />
      ...
      <Comment
          ...
          v-if="profileStore.replies.length !== 0 && globalState.activeProfileOption.value === 'replies'"
          :key="comment.id"
          :comment="comment"/>

      <Tweet
          v-for="tweet in profileStore.liked"
          v-if="profileStore.liked.length !== 0 && globalState.activeProfileOption.value === 'likes'"
          :key="tweet.id"
          :tweet="tweet"
      />

      <NoTweets v-if="profileStore.posts.length === 0 && globalState.activeProfileOption.value === 'posts'"
                :full_name="$page.props.user.full_name" class="my-5"/>

      <NoReplies v-if="profileStore.replies.length === 0 && globalState.activeProfileOption.value === 'replies'"
                 :full_name="$page.props.user.full_name" class="my-5"/>

      <NoLikes v-if="profileStore.liked.length === 0 && globalState.activeProfileOption.value === 'likes'"
               :full_name="$page.props.user.full_name" class="my-5"/>
    </section>
  </AuthenticatedLayout>
</template>
