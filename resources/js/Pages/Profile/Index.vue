...
profileStore.getFeed('posts', page.props.user.username);

const {posts, liked, replied} = storeToRefs(profileStore);
  ...
  <AuthenticatedLayout>
    ...
    <section class="grid">
      ...
      <Tweet
          ...
          v-if="globalState.activeProfileOption.value === 'posts'"
          :key="tweet.id"
          :tweet="tweet"
      />
      <NoTweets v-if="posts.length === 0 && globalState.activeProfileOption.value === 'posts'"
                :full_name="$page.props.user.full_name" class="my-5"/>

      <NoLikes v-if="liked.length === 0 && globalState.activeProfileOption.value === 'likes'"
               :full_name="$page.props.user.full_name" class="my-5"/>

      <NoReplies v-if="replied.length === 0 && globalState.activeProfileOption.value === 'replies'"
                 :full_name="$page.props.user.full_name" class="my-5"/>
    </section>
  </AuthenticatedLayout>
</template>
