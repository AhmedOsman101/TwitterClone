...
const formattedMessage = computed(() => {
  ...
  let startIndex = -1;
  if (props.notification.type === 'follow') startIndex = input.indexOf('started');
  if (props.notification.type === 'like') startIndex = input.indexOf('liked');
  if (props.notification.type === 'reply') startIndex = input.indexOf('replied');
...
<template>
  <div class="tweet pl-5 pt-5 thinBorder-b">
    ...
    <p class="tweetBody">
      <Link :href="route('profile.index', notification.username)" class="font-semibold hover:underline"
    ...
    <div class="actions">
      ...
      <button class="action_button group" @mouseenter="showTooltip(1)" @mouseleave="hideTooltip(1)">
        <i class="fa-solid fa-check"/>
        <Tooltip :message="'Mark as Read'" :visible="tooltip_1"/>
      </button>

      <button class="action_button group" @mouseenter="showTooltip(2)" @mouseleave="hideTooltip(2)">
